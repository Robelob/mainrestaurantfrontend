...
const foodGallery = importAll(
  require.context("../../img/food", false, /\.(jpe?g)$/)
);

class Home extends Component {
  render() {
    const imgKeys = Object.keys(foodGallery);
    const images = imgKeys.map((e) => (
      <Link to="/menu" key={e}>
        <img src={foodGallery[e]} alt={e} />
      </Link>
    ));
    ...
    return (
      <Fragment>
        <header
          className="landing-sub handwritten"
        ...
        <section className="home-section">
          <section className="food-teaser">{images}</section>
